...
PROJECT_METADATA_PREFIX = "# tmux-generator-project: "
...
def _quick_dict(quick: QuickCommandOptions) -> dict:
    return {"enabled": quick.enabled, "start": quick.start, "stop": quick.stop}


def _quick_from_dict(value: dict | None) -> QuickCommandOptions:
    value = value or {}
    return QuickCommandOptions(enabled=bool(value.get("enabled", False)), start=str(value.get("start", "")), stop=str(value.get("stop", "")))


def project_metadata(config: dict, quick: QuickCommandOptions) -> str:
    payload = {"version": PROJECT_METADATA_VERSION, "config": config, "quick": _quick_dict(quick)}
    encoded = base64.b64encode(json.dumps(payload, ensure_ascii=False, separators=(",", ":")).encode("utf-8")).decode("ascii")
    return PROJECT_METADATA_PREFIX + encoded


def import_script(script: str) -> dict:
    for line in script.splitlines():
        if not line.startswith(PROJECT_METADATA_PREFIX):
            continue
        encoded = line[len(PROJECT_METADATA_PREFIX) :].strip()
        try:
            payload = json.loads(base64.b64decode(encoded.encode("ascii"), validate=True).decode("utf-8"))
        except (ValueError, json.JSONDecodeError) as exc:
            raise ValueError("invalid tmux-generator metadata") from exc
        if payload.get("version") != PROJECT_METADATA_VERSION or not isinstance(payload.get("config"), dict) or not isinstance(payload.get("quick"), dict):
            raise ValueError("invalid tmux-generator metadata")
        quick = _quick_from_dict(payload["quick"])
        return {"config": payload["config"], "quick": _quick_dict(quick), "previous_quick": _quick_dict(quick)}
    raise ValueError("unsupported tmux-generator script")
...
def generate_script(config: dict, export_dir: str | Path, quick: QuickCommandOptions | None = None) -> str:
    ...
    lines = [
        "#!/usr/bin/env bash",
        "set -e",
        project_metadata(config, quick),
        "",
        "# Generated by tmux session orchestrator demo",
        f"# Export dir: {export_dir}",
        "# Pane order: top-to-bottom first, then left-to-right",
    ]
    if quick.enabled:
        delete = delete_quick_command_name(quick)
        lines.extend(
...
def export_files(config: dict, options: ExportOptions) -> ExportResult:
    ...
    if options.kind in {"sh", "both"}:
        ...
        files.append(path)
    _remove_previous_quick_commands(options)
    quick_paths = _install_quick_commands(config, files, options)
    return ExportResult(files=files, quick_commands=quick_paths, quick=resolve_quick_commands(config, options.quick))


def _remove_previous_quick_commands(options: ExportOptions) -> None:
    if not options.previous_quick or not options.previous_quick.start:
        return
    bin_dir = Path(options.bin_dir or Path.home() / ".local" / "bin").expanduser()
    for name in [options.previous_quick.start, options.previous_quick.stop, delete_quick_command_name(options.previous_quick)]:
        if not name:
            continue
        path = bin_dir / name
